...
function addMetricSample(row, score, value, outcome = {}, deadbandPct = 0.5) {
  ...
  if (Number.isFinite(horizon) && horizon > 0) {
    ...
    row.horizons[key] = h;
    addMetricSample(h, s, v, { ...outcome, horizonDays: null }, deadbandPct);
  }
}
...
function finalizeMetricRow(row, source = "") {
  ...
  row.ic = row.rankIC;
  row.effectiveN = new Set((row.effectiveKeys || []).filter(Boolean)).size || row.n;
  row.effectiveNMethod = "unique-decision-non-overlap";
  ...
  for (const horizon of Object.values(row.horizons || {})) {
    ...
    horizon.effectiveN = new Set((horizon.effectiveKeys || []).filter(Boolean)).size || horizon.n;
    horizon.effectiveNMethod = "unique-decision-non-overlap";
