...
const NYSE_REFERENCE_CALENDAR_PATH = new URL("../data/reference/nyse_calendar_2019_2028.json", import.meta.url);
let nyseReferenceCalendarCache;
...
function loadNyseReferenceCalendar() {
  if (nyseReferenceCalendarCache !== undefined) return nyseReferenceCalendarCache;
  try {
    const payload = JSON.parse(fs.readFileSync(NYSE_REFERENCE_CALENDAR_PATH, "utf8"));
    const byDate = new Map((payload.sessions || []).map((row) => [row.date, row]));
    nyseReferenceCalendarCache = { ...payload, byDate };
  } catch (error) {
    void error;
    nyseReferenceCalendarCache = null;
  }
  return nyseReferenceCalendarCache;
}

function nyseReferenceSession(ymd) {
  const calendar = loadNyseReferenceCalendar();
  return calendar?.byDate?.get(String(ymd || "").slice(0, 10)) || null;
}
...
const NYSE_SPECIAL_CLOSURES = new Map([
  ["2025-01-09", "National Day of Mourning for President Jimmy Carter"],
]);

export function nyseHolidayName(ymd) {
  const reference = nyseReferenceSession(ymd);
  if (reference && reference.isTradingDay === false) return reference.reason || reference.label || "NYSE closed";
  return nyseHolidayNameRuleBased(ymd);
}

export function nyseHolidayNameRuleBased(ymd) {
  const date = utcDateFromYmd(ymd);
  if (!date) return "";
  ...
  for (const [date, reason] of NYSE_SPECIAL_CLOSURES.entries()) holidays.set(date, reason);
  return holidays.get(ymdFromDate(date)) || "";
}

export function nyseHalfDayName(ymd) {
  const reference = nyseReferenceSession(ymd);
  if (reference?.isTradingDay && reference.isHalfDay) return reference.reason || reference.label || "NYSE early close";
  return nyseHalfDayNameRuleBased(ymd);
}

export function nyseHalfDayNameRuleBased(ymd) {
  const date = utcDateFromYmd(ymd);
  if (!date) return "";
  const year = date.getUTCFullYear();
  const dow = weekdayUtc(date);
  ...
  if (ymd === `${year}-12-24` && !nyseHolidayNameRuleBased(ymd)) return "Christmas Eve early close";
  return "";
}

export function isNyseTradingDay(ymd) {
  const reference = nyseReferenceSession(ymd);
  if (reference) return Boolean(reference.isTradingDay);
  return isNyseTradingDayRuleBased(ymd);
}

export function isNyseTradingDayRuleBased(ymd) {
  const date = utcDateFromYmd(ymd);
...
export function nyseSessionForYmd(ymd) {
  ...
  if (reference) {
    return {
      ymd,
      isTradingDay: Boolean(reference.isTradingDay),
      isHalfDay: Boolean(reference.isHalfDay),
      label: reference.label || (reference.isTradingDay ? reference.isHalfDay ? "半日市" : "正常交易日" : "休市"),
      reason: reference.reason || "",
      source: "pandas-market-calendars-reference",
    };
  }
  return nyseSessionForYmdRuleBased(ymd);
}

export function nyseSessionForYmdRuleBased(ymd) {
  const holiday = nyseHolidayNameRuleBased(ymd);
  if (holiday) return { ymd, isTradingDay: false, isHalfDay: false, label: "休市", reason: holiday };
  if (!isNyseTradingDayRuleBased(ymd)) return { ymd, isTradingDay: false, isHalfDay: false, label: "非交易日", reason: "Weekend or invalid date" };
  const halfDay = nyseHalfDayNameRuleBased(ymd);
...
function zonedDateTimeToUtc(ymd, hour, minute, timeZone) {
  const [year, month, day] = String(ymd || "").split("-").map(Number);
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const offset = timeZoneOffsetMs(new Date(timestamp), timeZone);
...
export function nyseSessionCloseAt(value) {
  const ymd = typeof value === "string" ? value.slice(0, 10) : ymdFromDate(new Date(value));
  const session = nyseSessionForYmd(ymd);
  if (!session?.isTradingDay) return null;
  return zonedDateTimeToUtc(ymd, session.isHalfDay ? 13 : 16, 0, "America/New_York");
}
...
function tickerTerms(ticker, companyName = "") {
  ...
  if (cleaned.length >= 3) {
    ...
    const genericCompanyWords = new Set([
      "common", "company", "corporation", "etf", "fund", "global", "group", "holdings", "international",
      "shares", "stock", "systems", "technologies", "technology", "trust",
    ]);
    for (const part of cleaned.split(/\s+/).filter((item) => item.length >= 4 && !genericCompanyWords.has(item.toLowerCase()))) {
...
export function semanticNewsOwnership(item = {}, context = {}) {
  ...
  const hits = terms.filter((term) => {
    ...
    if (ticker && ambiguousTickerTerms.has(ticker) && term === ticker.toLowerCase()) {
      const escaped = ticker.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      return new RegExp(`(^|[^A-Z0-9])\\$?${escaped}([^A-Z0-9]|$)`).test(rawText);
    }
...
export function inferHeadlineSubjectTicker(item = {}, candidates = []) {
  const titleVariants = [...new Set([item.article?.title, item.title, item.titleZh]
    .map((value) => String(value || "").replace(/\s+/g, " ").trim())
    .filter(Boolean))];
  if (!titleVariants.length) return null;
  const matches = [];
  for (const [variantIndex, title] of titleVariants.entries()) {
    const subjectZone = title.slice(0, 120).toLowerCase();
    for (const candidate of candidates || []) {
      const ticker = String(candidate?.ticker || "").toUpperCase().replace(/[^A-Z0-9.-]/g, "");
      if (!ticker) continue;
      const terms = [candidate.name, ...(candidate.aliases || [])]
        .map((term) => String(term || "").trim())
        .filter((term) => /[^\x00-\x7F]/.test(term) ? term.length >= 2 : term.length >= 4);
      for (const term of terms) {
        const index = subjectZone.indexOf(term.toLowerCase());
        if (index >= 0) matches.push({ ticker, term, index, variantIndex, rank: index + variantIndex * 2 });
      }
    }
  }
  if (!matches.length) return null;
  matches.sort((a, b) => a.rank - b.rank || b.term.length - a.term.length || a.ticker.localeCompare(b.ticker));
  const best = matches[0];
  return {
    ticker: best.ticker,
    matchedTerm: best.term,
    index: best.index,
    titleVariant: best.variantIndex,
    confidence: best.index <= 12 ? 95 : 85,
    method: "headline-leading-entity",
  };
}
