...
def parse_returns(payload: dict[str, Any]) -> tuple[list[float], list[float], list[str]]:
    ...
    for row in rows:
        if isinstance(row, dict):
            ...
            day = str(row.get("date") or row.get("asOf") or "")
        else:
            value = number(row)
            bench = 0.0
            day = ""
        if value is not None:
            ...
            dates.append(day)
    return returns, benchmark, dates


def quantstats_index(dates: list[str], length: int):
    import pandas as pd  # type: ignore

    index = pd.to_datetime(dates[:length], errors="coerce") if dates else None
    if index is None or len(index) != length or getattr(index, "isna", lambda: [])().any():
        index = pd.bdate_range(end=pd.Timestamp.today().normalize(), periods=length)
    return index


def maybe_quantstats_metrics(returns_pct: list[float], benchmark_pct: list[float], dates: list[str]) -> tuple[dict[str, Any] | None, str]:
    try:
        import pandas as pd  # type: ignore
        import quantstats as qs  # type: ignore
    except Exception as exc:
        return None, f"quantstats unavailable: {type(exc).__name__}: {exc}"
    index = quantstats_index(dates, len(returns_pct))
    returns = pd.Series([value / 100 for value in returns_pct], index=index)
    benchmark = pd.Series([value / 100 for value in benchmark_pct], index=index[: len(benchmark_pct)]) if benchmark_pct else None
    n = int(returns.count())
    try:
        metrics = {
            "n": n,
...
def main() -> int:
    ...
    returns_pct, benchmark_pct, dates = parse_returns(payload)
    native = native_metrics(returns_pct, benchmark_pct)
    quantstats_metrics, degradation = maybe_quantstats_metrics(returns_pct, benchmark_pct, dates)
    html_path = None
    if quantstats_metrics and payload.get("htmlPath"):
        try:
            ...
            qs.reports.html(
                pd.Series([value / 100 for value in returns_pct], index=quantstats_index(dates, len(returns_pct))),
                output=str(out),
                title="Market Pulse Historical Backtest",
            )
            html_path = str(out)
        except Exception as exc:
