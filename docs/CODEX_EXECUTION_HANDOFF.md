...
# Round 9 — Whole-market systematic screen (WP35)
...
- **35c — agent funnel integration.** Top `MARKET_SCREEN_FUNNEL_LIMIT` (default 50) enter the agent candidate pool with label `全市场因子筛选` (new funnel in the pool builder next to 因子候选). They receive NO scoring privilege — same gates, same data-quality thresholds, same actionable caps. Pool cap may need `maxCandidates` headroom (settings change → skill version bump with changeReason, owner-approved here).
- **35d — screen surface.** A "全市场筛选" panel (config-linked) showing the nightly top-50 with factor attributions and bar freshness, clearly labeled as funnel/research — not recommendations. Persist last N=10 screens (sectioned store) for later screen-vs-outcome evaluation.
- **35e — measurement hook.** Tag decisions whose only entry label is the screen funnel (`entryFunnel:"market-screen"`) so factor stats can split screen-sourced vs attention-sourced samples (same trackingReason-split pattern as WP24) — this is how we learn whether systematic surfacing adds value.
- **Verify**: nightly job completes <15 min incremental with zero lock errors; screen scores for 3 tickers hand-match the walk-forward scorer on the same bars; a no-event ticker (e.g., a quiet S&P member absent from all other funnels) appears in the pool with the screen label; agent run shows screen-sourced candidates passing through normal gates; funnel split visible in factor stats.

Owner decision recorded: universe v1 is S&P-membership-based (~503 names). Full-listing coverage (10k+ via company_tickers.json) stays event-driven through existing movers/news funnels until screen value is proven (35e evidence, ≥50 screen-sourced outcomes).
